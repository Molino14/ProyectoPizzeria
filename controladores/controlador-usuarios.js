...
async function recuperarUsuariosPorNombre(req, res, next) {
  ...
  if (!usuarios || usuarios.length === 0) {
    const error = new Error(
      "No se ha podido encontrar el usuario con ese nombre"
    );
    error.code = 404;
    return next(error);
  } else {
...
async function recuperarUsuarioPorEmail(req, res, next) {
  ...
  if (!usuario) {
    const error = new Error(
      "No se ha podido encontrar el usuario con ese email"
    );
    error.code = 404;
    return next(error);
  } else {
...
async function crearUsuario(req, res, next) {
  ...
  const { nombre, apellidos, fNacimiento, telefono, email, password } =
    req.body;
...
async function modificarUsuario(req, res, next) {
  ...
  const { telefono, email, password, puntos } = req.body; // Sacamos del body del request las propiedades que queremos modificar
  const idUsuario = req.params.uid;
  let usuario;
  try {
    usuario = await Usuario.findById(idUsuario);
  ...
  // Modificamos telefono, email, password y puntos.
...
async function loginUsuario(req, res, next) {
  ...
  if (!existeUsuario) {
    ...
    try {
      esValidoElPassword = await bcrypt.compare(
        password,
        existeUsuario.password
      );
    } catch (error) {
      const err = new Error(
        "No se ha podido realizar el login. Revise sus credenciales"
      );
    ...
    if (!esValidoElPassword) {
      ...
      try {
        token = jwt.sign(
          { idUsuario: existeUsuario.id, email: existeUsuario.email },
          process.env.JWT_KEY,
          { expiresIn: "1h" }
        );
      } catch (error) {
