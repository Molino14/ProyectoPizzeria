...
async function modificarUsuario(req, res, next) {
    ...
    const {
        ...
        password,
        puntos
    } = req.body; // Sacamos del body del request las propiedades que queremos modificar
    const idUsuario = req.params.uid; 
    let usuario;
    try {
        usuario = await Usuario.findById(idUsuario);
    ...
    console.log("PATCH realizado con éxito")
}
// Login del cliente
async function loginUsuario (req, res, next) {
    ...
    if (!existeUsuario) {
        ...
        if (!esValidoElPassword) {
            ...
            let token;
            try {
                token = jwt.sign({idUsuario: existeUsuario.id, email: existeUsuario.email}, 'superclave', {expiresIn: '1h'});
            } catch (error) {
